...
class CategoryActionTest extends WebTestCase
{
    ...
    public function testSuccess(): void
    {
        ...
        $response = $this->app()->handle(self::json('GET', '/v1/categories?id=' . $id, [], self::$HEADERS));
    ...
    {
        $response = $this->app()->handle(self::json('GET', '/v1/categories?id=', [], self::$HEADERS));
        $data = json_decode((string)$response->getBody(), true);
    ...
    {
        $response = $this->app()->handle(self::json('GET', '/v1/categories?id=IncorrectId', [], self::$HEADERS));
        $data = json_decode((string)$response->getBody(), true);
